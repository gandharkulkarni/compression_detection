...
int send_tcp_syn()
{
  ...
  uint8_t *send_ptr = total_data;
   for(uint16_t j = 0; j < config->udp_packets; j++){

      usleep(200);
    
      send_ptr = total_data + (j * config->udp_payload_size); //send_ptr points to the next payload section in the buffer
      if((sendto(udp_sockfd, send_ptr, config->udp_payload_size, MSG_CONFIRM, (const struct sockaddr *)&addr, sizeof(addr))) < 0){
